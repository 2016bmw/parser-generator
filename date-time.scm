...
(define (valid-date? parse-tree)
  (let ((year (car parse-tree))
        (month (cadr parse-tree))
        (day (caddr parse-tree)))
    (and (<= 1 month 12)
         (<= 1 day (days-in-month month year)))))

(define (valid-time? parse-tree)
  (let ((hour (car parse-tree))
        (minute (cadr parse-tree))
        (second (caddr parse-tree)))
    (and (<= 0 hour 23)
         (<= 0 minute 59)
         (<= 0 second 59))))
...
(define date-time-grammar
  (letrec* ((digits (lambda (digits)
                      (p:transform
                       (lambda (parse-tree)
                         (string->number
                          (apply string-append (map car (parse-tree-data parse-tree)))))
